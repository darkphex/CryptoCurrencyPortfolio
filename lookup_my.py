...
root.iconbitmap(r'C:\Users\p.ullrich\Documents\5_Work\5.1_Further training\5.1.2_Coding\Python\Basics\CryptoCurrencyPortfolio\bitcoin_94576.ico')
...
header_profit_loss_per = Label(root, text="P/L Per", bg="white", font = "Verdana 8 bold")
...
header_profit_loss_total = Label(root, text="Profit/Loss Total", bg="white", font = "Verdana 8 bold")
...
def lookup():
    ...
    headers = {
        ...
        'X-CMC_PRO_API_KEY': '36a15a0b-c2c3-484e-b566-224cef3d215f',
    }
    ...
    # api_request = requests.get("https://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?CMC_PRO_API_KEY=36a15a0b-c2c3-484e-b566-224cef3d215f")
    ...
    for x in dataObject:
        ...
        for coin in my_portfolio:
            if coin["sym"] == x["symbol"]:
                ...
                current_value = float(coin["amount_owned"]) * float(usd["price"])
                profit_loss = current_value - total_paid
                portfolio_profit_loss += profit_loss
                profit_loss_per_coin = float(usd["price"]) - float(coin["price_paid_per"])
                ...
                #print(x["name"])
                #print(" Current Price: ${0:.2f}".format(float(usd["price"])))
                #print(" Protif/Loss Per Coin: ${0:.2f}".format(float(profit_loss_per_coin)))
                ...
                #print(" Protif/Loss: ${0:.2f}".format(float(profit_loss)))
                #print("------------------------------------------------")
                ...
                current_price = Label(root, text="${0:.2f}".format(float(usd["price"])), bg="white")
                ...
                one_hr_change = Label(root, text="{0:.2f}%".format(float(usd["percent_change_1h"])), bg="silver", fg=red_green(float(usd["percent_change_1h"])))
                one_hr_change.grid(row=row_count, column=5, sticky=N+S+E+W)

                tf_hr_change = Label(root, text="{0:.2f}%".format(float(usd["percent_change_24h"])), bg="white", fg=red_green(float(usd["percent_change_24h"])))
                tf_hr_change.grid(row=row_count, column=6, sticky=N+S+E+W)

                seven_day_change = Label(root, text="{0:.2f}%".format(float(usd["percent_change_7d"])), bg="silver", fg=red_green(float(usd["percent_change_7d"])))
                ...
                profit_loss_total = Label(root, text="${0:.2f}".format(float(profit_loss)), bg="white", fg=red_green(float(profit_loss)))
